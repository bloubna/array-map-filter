...
function filterOffensiveComments(comments, bannedWords) {
  const results = comments.filter(comment => {
    if (!(comment.includes(bannedWords[0].toLowerCase()) || comment.includes(bannedWords[1].toLowerCase()) || comment.includes(bannedWords[0].toUpperCase()) || comment.includes(bannedWords[1].toUpperCase()))){
          return comment;
    }
  });
  return results;
